...
# Known arch rows from scan v1.3
KNOWN_ARCH_ROWS = {2} | set(range(43, 59)) | {99}  # 18 rows
KNOWN_NORMAL_ROWS = set(range(NUM_ROWS)) - KNOWN_ARCH_ROWS  # 84 rows


def selective_erase(isp, cmd):
    """Execute a selective erase command (GRPBE, GLBBE, or ARCHBE).
    Same protocol as bulk_erase but with different command opcode."""
    cmd_names = {GRPBE: "GRPBE", GLBBE: "GLBBE", ARCHBE: "ARCHBE"}
    name = cmd_names.get(cmd, f"0x{cmd:02X}")
    print(f"  Selective erase: {name}...")
    isp.goto_idle()
    isp.change_state()           # IDLE -> SHIFT
    isp.shift_command(cmd)
    isp.change_state()           # SHIFT -> EXECUTE
    isp._execute(wait_time=T_BEW)  # same 300ms wait as bulk erase
    isp.goto_idle()
    print(f"  Selective erase {name} done.")


def test_selective_erase(isp):
    """Test selective erase commands to confirm which rows belong to which section.

    Protocol per test:
      1. UBE (bulk erase) — clean slate
      2. Write known pattern to ALL 102 rows
      3. Selective erase (ARCHBE / GRPBE / GLBBE)
      4. Read all 102 rows
      5. Compare: erased rows = belong to that section

    Cost: 3 bulk erases + 3×102 writes + 3×102 reads + 3 selective erases = ~312 erase cycles
    (Actually only 3+3=6 erase operations, writes don't count as erase cycles)
    """
    # Pattern: all zeros (maximally programmed) — easy to distinguish from erased (all 1s)
    pat_h, pat_l = 0x0000000000, 0x0000000000

    tests = [
        ("ARCHBE", ARCHBE, "Architecture array (ORP/IO config + global)"),
        ("GRPBE",  GRPBE,  "Global Routing Pool array"),
        ("GLBBE",  GLBBE,  "Generic Logic Block array"),
    ]

    all_results = {}

    for test_name, erase_cmd, description in tests:
        print()
        print("=" * 60)
        print(f"SELECTIVE ERASE TEST: {test_name} — {description}")
        print("=" * 60)

        # Step 1: Bulk erase
        print("\n  Step 1: Bulk erase (UBE)...")
        isp.bulk_erase()

        # Step 2: Write pattern to all rows (skip stuck bits — write 0x0000000000)
        print(f"  Step 2: Writing pattern to all {NUM_ROWS} rows...")
        for row in range(NUM_ROWS):
            isp.write_row(row, pat_h, pat_l)
            if row % 10 == 0:
                sys.__stdout__.write(f"\r    Writing row {row}/{NUM_ROWS}...")
                sys.__stdout__.flush()
        sys.__stdout__.write(f"\r    Writing done.                \n")
        sys.__stdout__.flush()

        # Verify a few rows before selective erase
        print("  Step 2b: Spot-check writes...")
        for check_row in [0, 50, 101]:
            rd_h, rd_l = isp.read_row_fast(check_row)
            # Normal rows should read back exact pattern
            # Arch rows will have stuck bits, but non-stuck bits should be 0
            if check_row in KNOWN_ARCH_ROWS:
                # Mask out stuck bits for comparison
                print(f"    Row {check_row} (arch): H={fmt_hex(rd_h,40)} L={fmt_hex(rd_l,40)}")
            else:
                match = (rd_h == pat_h and rd_l == pat_l)
                print(f"    Row {check_row}: H={fmt_hex(rd_h,40)} L={fmt_hex(rd_l,40)} {'OK' if match else 'MISMATCH'}")

        # Step 3: Selective erase
        print(f"\n  Step 3: {test_name} selective erase...")
        selective_erase(isp, erase_cmd)

        # Step 4: Read all rows and classify
        print(f"  Step 4: Reading all {NUM_ROWS} rows...")
        erased_rows = []
        programmed_rows = []
        partial_rows = []
        ...
        for row in range(NUM_ROWS):
            ...
            if rd_h == ERASED_HIGH and rd_l == ERASED_LOW:
                erased_rows.append(row)
            elif rd_h == pat_h and rd_l == pat_l:
                programmed_rows.append(row)
            else:
                # Partial — some bits erased, some not (arch rows with stuck bits)
                # Check if programmable bits are erased (ignoring stuck positions)
                # Stuck positions: bits 7,8,17,18,27,28,37 on arch rows
                stuck_mask = 0
                for b in [7, 8, 17, 18, 27, 28, 37]:
                    stuck_mask |= (1 << b)

                h_fuse = rd_h & ~stuck_mask & 0x3FFFFFFFFF
                l_fuse = rd_l & ~stuck_mask & 0x3FFFFFFFFF
                erased_fuse_h = ERASED_HIGH & ~stuck_mask & 0x3FFFFFFFFF
                erased_fuse_l = ERASED_LOW & ~stuck_mask & 0x3FFFFFFFFF

                if h_fuse == erased_fuse_h and l_fuse == erased_fuse_l:
                    # Programmable bits are erased — this row was affected
                    erased_rows.append(row)
                elif h_fuse == 0 and l_fuse == 0:
                    # Programmable bits still programmed — not affected
                    programmed_rows.append(row)
                else:
                    partial_rows.append(row)

                print(f"    Row {row:3d}: H={fmt_hex(rd_h,40)} L={fmt_hex(rd_l,40)} "
                      f"(fuse_h={fmt_hex(h_fuse,40)} fuse_l={fmt_hex(l_fuse,40)})")

            # Progress
            if row % 20 == 0:
                sys.__stdout__.write(f"\r    Reading row {row}/{NUM_ROWS}...")
                sys.__stdout__.flush()

        sys.__stdout__.write(f"\r    Reading done.                \n")
        sys.__stdout__.flush()

        # Step 5: Report
        print(f"\n  Results for {test_name}:")
        print(f"    Erased (affected):     {len(erased_rows)} rows: {erased_rows}")
        print(f"    Programmed (untouched): {len(programmed_rows)} rows")
        print(f"    Partial/unclear:       {len(partial_rows)} rows: {partial_rows}")

        # Compare with hypothesis
        if test_name == "ARCHBE":
            expected = KNOWN_ARCH_ROWS
            label = "arch"
        elif test_name == "GRPBE":
            expected = None  # unknown split
            label = "GRP"
        elif test_name == "GLBBE":
            expected = None
            label = "GLB"

        if expected:
            match_set = set(erased_rows)
            if match_set == expected:
                print(f"    ✓ MATCHES hypothesis! {label} rows = {sorted(expected)}")
            else:
                extra = match_set - expected
                missing = expected - match_set
                if extra:
                    print(f"    Extra erased (not expected): {sorted(extra)}")
                if missing:
                    print(f"    Missing (expected erased):   {sorted(missing)}")

        all_results[test_name] = {
            "erased": erased_rows,
            "programmed": programmed_rows,
    ...
    print()
    print("=" * 60)
    print("SELECTIVE ERASE SUMMARY")
    print("=" * 60)
    for name, res in all_results.items():
        print(f"  {name}: {len(res['erased'])} erased, {len(res['programmed'])} kept, {len(res['partial'])} partial")

    # Check for overlap / coverage
    arch_set = set(all_results["ARCHBE"]["erased"])
    grp_set = set(all_results["GRPBE"]["erased"])
    glb_set = set(all_results["GLBBE"]["erased"])
    all_set = set(range(NUM_ROWS))

    print(f"\n  ARCH ∩ GRP:  {sorted(arch_set & grp_set) if arch_set & grp_set else 'empty'}")
    print(f"  ARCH ∩ GLB:  {sorted(arch_set & glb_set) if arch_set & glb_set else 'empty'}")
    print(f"  GRP ∩ GLB:   {sorted(grp_set & glb_set) if grp_set & glb_set else 'empty'}")
    print(f"  ARCH ∪ GRP ∪ GLB = {len(arch_set | grp_set | glb_set)} rows (of {NUM_ROWS})")
    uncovered = all_set - (arch_set | grp_set | glb_set)
    if uncovered:
        print(f"  Uncovered rows: {sorted(uncovered)}")
    ...
    result_file = os.path.join(RESULTS_DIR, "selective_erase.json")
...
def scan_all_rows(isp):
    ...
    for row in range(NUM_ROWS):
        ...
        if rd_h == pat_h and rd_l == pat_l:
            pass_count += 1
            # Progress spinner — terminal only (not log)
            spinner = "|/-\\"
            sys.__stdout__.write(f"\r  Row {row:3d}/{NUM_ROWS} {spinner[row % 4]} ({pass_count} pass, {len(fail_rows)} fail)")
            sys.__stdout__.flush()
        else:
            sys.__stdout__.write(f"\r  Row {row:3d}/{NUM_ROWS}                                    \n")
            sys.__stdout__.flush()
            xor_h = pat_h ^ rd_h
            xor_l = pat_l ^ rd_l
            stuck_bits_h = []
...
def main():
    ...
    parser.add_argument('--selective-erase', action='store_true',
                        help='Test ARCHBE/GRPBE/GLBBE to map row sections (3 erase cycles)')
    ...
    if not (args.phase1 or args.phase2 or args.phase2_auto or args.diag_bits or args.diag_speed or args.diag_row or args.scan or args.selective_erase):
        parser.print_help()
        print("\nExample:")
    ...
    try:
        ...
        if args.selective_erase:
            test_selective_erase(isp)
